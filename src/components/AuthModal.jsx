    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      ...
      <div className="relative z-10 w-full max-w-md p-6 sm:p-8 rounded-3xl bg-slate-900/95 backdrop-blur-xl border border-white/20 shadow-2xl space-y-5 my-8 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <button type="submit" disabled={loading} className="w-full btn btn-primary py-3 text-xs shadow-lg shadow-amber-500/30 cursor-pointer font-extrabold">
            <span>{loading ? 'Processing...' : isRegister ? `Register ${role.toUpperCase()} Account` : `Sign In as ${role.toUpperCase()}`}</span>
